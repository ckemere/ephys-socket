...
// STFT canvas + remote/dwt_plot.py so all three viewers look the same.
...
void DwtCanvas::refresh()
{
    ...
    int laneOffset = selectedLane * ns * 2;   // lane-major payload: re,im per bin
    ...
    // First pass: convert each new column's complex int32 payload to magnitude
    ...
    for (int c = colOffset; c < nNew; ++c)
    {
        ...
        if ((int)col.samples.size() < laneOffset + ns * 2) {
        ...
        float gmax = 0.0f;
        for (int s = 0; s < ns; ++s) {
            double re = (double)col.samples[laneOffset + s * 2 + 0];
            double im = (double)col.samples[laneOffset + s * 2 + 1];
            float mag = (float)std::sqrt(re * re + im * im);
            m[s] = mag;
...
void DwtCanvas::paint(Graphics& g)
{
    ...
    // engine's decimation, so we don't claim an exact dt -- just orientation).
